...
The default demo runs in deterministic offline mode and does not require an OpenAI API key. Set `LEAP_ENABLE_AI=1` and `OPENAI_API_KEY` only when intentionally testing optional draft AI enrichment.
...
4) Optional: enable draft AI enrichment
...
- For the standard demo sample: `bluebook generate ./sample_project`
- For the larger sample set: `bluebook generate ./sample_project_50`
...
- Permanent for future shells: `setx LEAP_ENABLE_AI "1"` and `setx OPENAI_API_KEY "sk-..."` (then open a new PowerShell)
- Or just for this session: `$env:LEAP_ENABLE_AI = "1"` and `$env:OPENAI_API_KEY = "sk-..."`
...
- `bluebook generate .\sample_project`
- Larger sample set: `bluebook generate .\sample_project_50`
...
- Missing API key or 401 errors:
    - This only applies when `LEAP_ENABLE_AI=1`. Set `OPENAI_API_KEY` correctly in the same shell session or unset `LEAP_ENABLE_AI` for offline mode.
...
- `pip install -e .`
- `bluebook generate ./sample_project --clean-build --workers 4`
- `open docs/_build/index.html`
