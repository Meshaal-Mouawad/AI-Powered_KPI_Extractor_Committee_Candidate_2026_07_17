...
The dissertation investigates whether a literate-programming approach can make those forms of KPI knowledge inspectable together. LEAP derives structured evidence from source artifacts and publishes an Interactive Bluebook that links the available business definition, formula, variables, source location, provenance, ownership, and review signals for a KPI. Executable source remains authoritative for what a system computes; the Bluebook is the shared, traceable representation through which stakeholders can examine that evidence.
...
This repository lets the committee inspect the technical intervention behind that proposition:
